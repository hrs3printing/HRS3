  ...
  useEffect(() => {
    const fetchCats = async () => {
      try {
        ...
        if (cats.length > 0) {
          // If a slug is provided in the URL, find that category, otherwise default to the first one
    ...
    <PageShell>
      ...
      <PageContent className="!pt-0 sm:!pt-0 !px-0 sm:!px-8 max-w-[1600px] mx-auto">
        <div className="flex gap-0 sm:gap-16 min-h-[75vh]">
          ...
          {/* RIGHT: SUB-CATEGORIES (EXPANSIVE & CLEAN) */}
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-8">
                ...
                {(activeCategory?.subCategories || []).map((sub, idx) => (
                  <Link
                    ...
                    to={`/catalog/${encodeURIComponent(activeCategory.name.toLowerCase().replace(/\s+/g, "-"))}?sub=${encodeURIComponent(sub.name)}`}
                    className="group block relative animate-fadeUp"
